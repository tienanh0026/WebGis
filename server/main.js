...
app.get("/api/search-provinces", (req, res) => {
  client.query(
    `select id_1, name_1 from vnm_adm1 order by name_1 ;`,
    (err, result) => {
      if (err) {
...
app.get("/api/search-districts", (req, res) => {
  client.query(
    `select name_1,id_1, id_2, name_2 from vnm_adm2 order by id_2 ;`,
    (err, result) => {
      if (err) {
        return console.error("Error running query", err);
      }
...
app.get("/api/get-geometry-vietnam/provinces", (req, res) => {
  client.query(
    `SELECT name_1,st_assvg(geom),type_1 from vnm_adm1 order by id_1;`,
    (err, result) => {
      if (err) {
        return console.error("Error running query", err);
      }
...
app.get("/api/get-geometry-vietnam/district", (req, res) => {
  client.query(
    `select id_1, name_1, id_2, name_2,type_2,st_assvg(geom) from vnm_adm2 order by id_2`,
    (err, result) => {
      if (err) {
        return console.error("Error running query", err);
      }
...
app.post("/api/get-geometry-vietnam/district-new", (req, res) => {
  ...
  if (scale > 16 && scale < 17.5) {
    reduce_para = 1.1;
  }
  if (scale > 17.5 && scale < 19) {
    reduce_para = 1.05;
  }
  console.log("xmin:", xmin, " xmax: ", xmax, " screen_ex: ", screen_extend);
  var text = "";
  var a = text.concat(
    "(ST_XMin(ST_Collect(geom))*15+(ST_XMax(ST_Collect(geom))-ST_XMin(ST_Collect(geom)))*15/2) >",
    xmin,
    " and (ST_XMin(ST_Collect(geom))*15+(ST_XMax(ST_Collect(geom))-ST_XMin(ST_Collect(geom)))*15/2) <",
    xmax,
    " and (ST_YMax(ST_Collect(geom))*-15-abs((ST_YMax(ST_Collect(geom))-ST_YMin(ST_Collect(geom))))*-15/2) >",
    ymin,
    " and (ST_YMax(ST_Collect(geom))*-15-abs((ST_YMax(ST_Collect(geom))-ST_YMin(ST_Collect(geom))))*-15/2) <",
    ymax
  );
  var b =
    "select id_1,name_1,id_2, name_2,type_2,ST_XMin(ST_Collect(geom))*15 as xmin,ST_XMax(ST_Collect(geom))*15 as xmax, ST_YMin(ST_Collect(geom))*-15 as ymin,ST_YMax(ST_Collect(geom))*-15 as ymax, st_assvg(geom) from vnm_adm2 group by id_1,name_1,id_2,name_2,type_2,geom having ";
  var c = b.concat(a);
  console.log(c);
  //console.log(c);
  client.query(c, (err, result) => {
    ...
    console.log("layer Quận: ban đầu: ", result.rows[0].st_assvg.length);
    var new_svg1 = reduce_points(result.rows, distance_district * reduce_para);
    for (var a = 0; a < result.rows.length; a++) {
      result.rows[a].st_assvg = new_svg1[a];
    }
    console.log("layer Quận: sau khi loại: ", result.rows[0].st_assvg.length);
...
app.post("/api/get-geometry-vietnam/province-new", (req, res) => {
  ...
  var b =
    "select id_1,name_1,type_1,ST_XMin(ST_Collect(geom))*15 as xmin,ST_XMax(ST_Collect(geom))*15 as xmax, ST_YMin(ST_Collect(geom))*-15 as ymin,ST_YMax(ST_Collect(geom))*-15 as ymax, st_assvg(geom) from vnm_adm1 group by id_1,name_1,type_1, geom having ";
  var c = b.concat(a);
  //console.log(c);
  //console.log(c);
  ...
  client.query(c, (err, result) => {
    ...
    console.log("layer Tỉnh: ban đầu: ", result.rows[0].st_assvg.length);
    var new_svg1 = reduce_points(result.rows, distance_province * reduce_para);
    for (var a = 0; a < result.rows.length; a++) {
      result.rows[a].st_assvg = new_svg1[a];
    }
    console.log("layer Tỉnh: sau khi loại: ", result.rows[0].st_assvg.length);
...
app.post("/api/get-geometry-vietnam/communes-new", (req, res) => {
  const { screen, scale } = req.body;
  console.log(screen);
  var xmin = screen[0].x;
  var xmax = screen[1].x;
  ...
  // var center_x = xmin + (xmax-xmin)/2;
  // var center_y = ymax + (Math.abs(ymin-ymax)/2);
  // if(scale > 20 && scale < 21){
  //     screen_extend = 2.3;
  // }
  // else if(scale > 22 && scale < 24){
  //     screen_extend = 2.5;
  // }
  // else if(scale > 24){
  //     screen_extend = 2.8;
  ...
  var b =
    "select id_2,name_2,id_3, name_3,type_3,ST_XMin(ST_Collect(geom))*15 as xmin,ST_XMax(ST_Collect(geom))*15 as xmax, ST_YMin(ST_Collect(geom))*-15 as ymin,ST_YMax(ST_Collect(geom))*-15 as ymax, st_assvg(geom) from vnm_adm3 group by id_2,name_2,id_3,name_3,type_3,geom having ";
  var c = b.concat(a);
  //console.log(c);
  //console.log(c);
  client.query(c, (err, result) => {
    ...
    // }
    // console.log("layer Quận: sau khi loại: ",result.rows[0].st_assvg.length)
    res.json({ screen_reduce_communes: result.rows });
    //console.log(result.rows.length)
    //console.log(a)
    //district_distance = avg_distance(result.rows)
  });
...
app.post("/api/get-geometry-vietnam/commune", (req, res) => {
  ...
  district = "'" + district + "'";
  var b =
    "select id_1 , name_1, id_2, name_2, id_3, name_3,type_3, st_assvg(geom) from vnm_adm3 where name_2=";
...
app.get("/api/get-geometry-vietnam/province_bbox", (req, res) => {
  ...
  client.query(
    `select id_1,name_1,ST_Area(ST_Transform(geom, 2044))/1000000 As area ,ST_XMin(ST_Collect(geom))*15 as xmin,ST_XMax(ST_Collect(geom))*15 as xmax,
    ST_YMin(ST_Collect(geom))*-15 as ymin,ST_YMax(ST_Collect(geom))*-15 as ymax from vnm_adm1 group by id_1,name_1,geom order by id_1 `,
    (err, result) => {
      if (err) {
        return console.error("Error running query", err);
      }
...
app.get("/api/get-geometry-vietnam/district_bbox", (req, res) => {
  ...
  client.query(
    `select id_1,name_1,id_2, name_2,type_2,ST_Area(ST_Transform(geom, 2044))/1000000 As area,ST_XMin(ST_Collect(geom))*15 as xmin,ST_XMax(ST_Collect(geom))*15 as xmax,
    ST_YMin(ST_Collect(geom))*-15 as ymin,ST_YMax(ST_Collect(geom))*-15 as ymax from vnm_adm2 group by id_1,name_1,id_2,name_2,type_2,geom order by id_2 `,
    (err, result) => {
      if (err) {
        return console.error("Error running query", err);
      }
...
app.get("/api/get-geometry-vietnam/commune_bbox", (req, res) => {
  //const {scale} = req.body;
  //console.log(scale);

  client.query(
    `select id_1,name_1,id_2, name_2,id_3,name_3,type_3,ST_Area(ST_Transform(geom, 2044))/1000000 As area,ST_XMin(ST_Collect(geom))*15 as xmin,ST_XMax(ST_Collect(geom))*15 as xmax,
        ST_YMin(ST_Collect(geom))*-15 as ymin,ST_YMax(ST_Collect(geom))*-15 as ymax from vnm_adm3 group by id_1,name_1,id_2,name_2,id_3,name_3,type_3,geom order by id_3 `,
    (err, result) => {
      if (err) {
        return console.error("Error running query", err);
      }
      res.json({ svg_commune_bbox: result.rows });
      //vietnam_distance = avg_distance(result.rows)
    }
  );
});
